...
from ViT_LRP_swin import window_reverse
import pdb
...
def eval_batch(image, labels, index):
    total_time = None
    # Save input image
    if args.save_img:
        ...
        print(os.path.join(saver.results_dir, 'input/{}_mask.png'.format(index)))
...
#     one_hot.backward(retain_graph=True)
    ...
    for i in tqdm(range(8)):
        for j in range(8):
            Res_ = lrp.generate_LRP(image.cuda(), start_layer=start_layer, method="perturb_all", index=cls_index, k=i*8+j)
...
#         break
    ...
    correct, labeled, inter, union, ap, f1, pred, target, total_time = eval_batch(images, labels, batch_idx)
    if total_time:
